...
def data():
    ...
    #选取torchtext中的文本分类数据‘AG_NEWS'即新闻主题分类数据，保存在指定目录下
    #并将数据映射后的训练和验证数据加载到内存中
    train_dataset, test_dataset = AG_NEWS(root='./data/', split=('train', 'test'))
train_dataset = iter(AG_NEWS(root='../data/', split=('train')))
print(next(train_dataset))
# train_dataset=pd.read_csv('../data/AG_NEWS.data/train.csv')
# test_dataset=pd.read_csv('../data/AG_NEWS.data/train.csv')

#指定BATCH_SIZE的大小
BATCH_SIZE=16

#进行可用设备监测，有GPU优先使用GPU
...
#获得整个语料包含的不同词汇总数
VOCAB_SIZE=len(train_dataset.get_vocab())#.get_vocab()
#指定词嵌入的维度
EMBED_DIM=32
#获得类别总数
NUN_CLASS=len(train_dataset.get_vocab())
#实例化模型
...
def generate_batch(batch):
    ...
    形如：[(sample1,label1),(sample2,label2),(sample3,label3),...,(sampleN,labelN)]
    :return:样本张量和标签各自的列表形式（张量），形如：
    text=tensor([sample1,sample2,sample3,...,sampleN])
    label=tensor([label1,label2,label3,...labelN])
    '''
    #从batch中获得标签张量
...
for epoch in range(N_EPOCHS):
    ...
    print(f'\tLoss:{valid_loss:.4f}(valid)\t|\tAcc:{valid_acc*100:.1f}%(train)')
