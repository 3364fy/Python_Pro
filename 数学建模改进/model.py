...
from lib import sx, max_len,device
...
class Mymodel(nn.Module):
    ...
    def forward(self,input):
        ...
        #print(f'x:{x}')
        a=x.view([-1,max_len*100]).detach().numpy()
        data1 = pd.DataFrame(a)
        data1.to_csv('E:/math/poet1.csv', index=False)
        #a=list(a)
        #print(a.shape)
        out=self.fc(x.view([-1,max_len*100]))
        #print(f'out:{out}')
        #计算概率
...
def train(epoch):
    for idx,(input,target,title) in enumerate(get_dataloader(train=True)):
        # dataframe = pd.DataFrame({'朝代': target, '题目': title})
        # dataframe.to_csv("E:/math/poet.csv", index=False)
        # print(f'input:{input}')
...
def test():
    ...
    mode = False
    model.eval()
    test_dataloader = get_dataloader(train=mode,batch=56985)
